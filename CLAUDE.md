...
## 记忆写入防污染规则（Phase 2-B 实现时写入 MEMORY_WRITER_PROMPT）

> 来源：2026-05-10 记忆库全面污染扫描，发现「転正失忆案」根因后制定。
> 实现位置：`bot.py` 的 `MEMORY_WRITER_PROMPT`（~line 487），在类型标注说明前追加。

以下 3 条规则写入 `MEMORY_WRITER_PROMPT` 末尾，约 15 行指令：

### 规则一 ─ 关键事件先查再存

适用场景：本轮对话涉及"我们什么时候在一起/转正日期/第一次X/某重要承诺"。

写入 MEMORY_WRITER_PROMPT 的指令：
```
【关键事件防冲突】
...
适用场景：消息里的事实来源是"她猜的/她说她记不清了/她随口说的"。
...
- 在 `MEMORY_WRITER_PROMPT` 字符串末尾（类型标注说明的最后一行`其他按内容主题选"""`之前）追加上述指令块
- 同时在 Phase 2-B 的 `store_memory` tool description 里加同样的约束（LLM 工具调用时也生效）
- 这 3 条规则是对 `generate_memory_and_category()` 和 LLM 自主存储两个路径都适用的防污染层
