...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    response = client.beta.messages.create(  # 使用beta API支持Code Execution
        ...
        max_tokens=2048,
        system=system,
        betas=["code-execution-2025-08-25"],  # 启用Code Execution
        tools=[
            {"type": "web_search_20260209", "name": "web_search"},
            {"type": "web_fetch_20260209", "name": "web_fetch", "max_uses": 5},  # 加入web_fetch
            {"type": "code_execution_20250825", "name": "code_execution"}  # 加入代码执行能力
        ],
        messages=history
    )
